...
- **Read the prod cluster**: `cluster-query.yml` (`workflow_dispatch`) is self-service read-only `kubectl` for **any** repo with an Actions-write token on FuzeInfra — consumers verify their own deploys with it instead of asking a human to relay `kubectl` output (`docs/consuming-repos/CLUSTER_QUERY.md`). Mutating/exec verbs, `Secret` reads and `--raw` are refused: FuzeInfra's job logs are **public**, so a read whose output is a credential leaks it (`tests/test_cluster_query_guard.py`).
