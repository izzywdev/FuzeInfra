...
( echo "[setup] pip"; pip install --quiet --no-input yamllint check-jsonschema pyyaml || pip install --quiet --no-input --break-system-packages yamllint check-jsonschema pyyaml || true ) &
...
for b in gh kubeconform kubeseal helm yamllint check-jsonschema; do
