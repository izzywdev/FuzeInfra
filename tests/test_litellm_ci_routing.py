...
def test_the_credential_gate_is_on_resolution_not_on_one_secrets_name():
    ...
    # Passing `secrets.LITELLM_CI_KEY` as the `litellm-ci-key:` input to llm-endpoint is
    # the correct pattern — the action resolves credential availability internally.
    # What is forbidden is using it as a gate condition (the old skip-green pattern).
    bad_refs = [
        ln.strip() for ln in live.splitlines()
        if "secrets.LITELLM_CI_KEY" in ln and "litellm-ci-key:" not in ln
    ]
    assert not bad_refs, (
